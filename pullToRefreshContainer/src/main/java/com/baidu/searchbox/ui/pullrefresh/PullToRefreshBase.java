...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (absDiff > mTouchSlop || isPullRefreshing() || isPullLoading())  {
                ...
                if (isPullRefreshEnabled() && isReadyForPullDown()) {
                    ...
                    mIsHandledTouchEvent = (Math.abs(getScrollYValue()) > 0 || deltaY > 0.5f);  //SUPPRESS CHECKSTYLE
    ...
    public void setHeaderBackgroundColor(int resId) {
        if (mHeaderLayout != null) {
            mHeaderLayout.setHeaderBackgroundColor(getResources().getColor(resId));
        }
    }
    ...
    public void setFooterBackgroundColor(int resId) {
        if (mFooterLayout != null) {
        	mFooterLayout.setHeaderBackgroundColor(getResources().getColor(resId));
        }
    }
    ...
    public void setFooterBackgroundResource(int resId) {
        if (mFooterLayout != null) {
        	mFooterLayout.setHeaderBackgroundResource(resId);
        }
    }
    ...
    public void setHeaderBackgroundResource(int resId) {
        if (mHeaderLayout != null) {
            mHeaderLayout.setHeaderBackgroundResource(resId);
        }
    }
     ...
     * @param callbackRunnable 完成下拉操作后的回调
     */
    public void doPullRefreshing(final boolean smoothScroll, final long delayMillis, final Runnable callbackRunnable) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
    ...
    public void doPullRefreshing(final boolean smoothScroll, final long delayMillis) {
        doPullRefreshing(smoothScroll, delayMillis, null);
    }
    
    /**
     * 显示下拉刷新的状态，这个方法与{@link #doPullRefreshing(boolean, long)}不同的是，该方法不会触发刷新的回调动作，
     * 只是显示下拉刷新的状态
     * 
     * @param smoothScroll 表示是否有平滑滚动，true表示平滑滚动，false表示无平滑滚动
     * @param delayMillis 延迟时间
    ...
    public void setEmptyView(View emptyView) {
        if (mRefreshableViewWrapper != null) {
            ...
            mRefreshableViewWrapper.addView(emptyView, width, height);
        }
    } 
     ...
     * 开始刷新，当下拉松开后被调用
     * 
     * @param doRealRefresh 是否开始真正的刷新动作，如果为false的话，只会有下拉刷新的UI变化，不会有实际的刷新回调
     */
    private void startRefreshing(boolean doRealRefresh) {
        // 如果正在刷新
        if (isPullRefreshing()) {
            return;
        }
